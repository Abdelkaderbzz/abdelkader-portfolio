...
const Projects = () => {
  ...
  const projectAnimItems = useStaggeredAnimation(projectsData.length, 0.1);
          ...
          {projectsData.map((project, index) => {
            ...
            return (
              <div 
                key={project.id}
                ref={ref}
                className={`group glass rounded-2xl overflow-hidden transition-all duration-500 hover:shadow-lg hover:shadow-primary/10 ${isVisible ? 'animate-fade-in' : 'opacity-0'}`}
                style={projectAnimItems[index].style}
              >
